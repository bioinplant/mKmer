...
option_list <- list(
    make_option(
        c("-i", "--input"),
        type = "character",
        help = "Input histo files, separated by commas",
        metavar = "character"
    ),
    make_option(
        c("-o", "--out"),
...
for (i in seq_along(file_paths)) {
    ...
    data <- data.frame(
        Coverage = kmer_data$V1,
        Frequency = kmer_data$V2,
        Source = paste0(kmer_size, "-mer")
    )
    
    # Update color mapping and combined data
    color_mapping[paste0(kmer_size, "-mer")] <- colors[i]
    combined_data <- rbind(combined_data, data)
    
    # Track first peak coverage and last peak frequency
    if (i == 1) {
...
peak_info <- combined_data %>%
    group_by(Source) %>%
    summarize(
        peak_Coverage = Coverage[which.max(Frequency)],
...
# Save plot
ggsave(
    paste0(output_path, "KmerFrequency.png"),
    plot = pic_dna,
    width = 15,
    height = 12,
    units = "in",
