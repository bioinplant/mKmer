...
def main(argv):
    ...
    interproscan_tool(interproscan, fasta_file, output, cluster)
...
def interproscan_tool(interproscan, fasta_file, output, cluster):
    output_file = os.path.join(output, f"cluster{cluster}_AAseq.fasta.tsv")
    command_interproscan = f'{interproscan} -i {fasta_file} -o {output_file} -f tsv -goterms'
