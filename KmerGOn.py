...
# Command 1: Run Tomtom with MEME R script
def run_tomtom_meme_r(cluster, input, output, db):
    command_01_script = os.path.join(os.path.dirname(__file__), 'KmerGOn_01.R')
    command_01 = [
...
def run_ama_gomo_sh(meme_file, db, cluster, input, output):
    command_02_script = os.path.join(os.path.dirname(__file__), 'KmerGOn_02.sh')
    command_02 = [
        'bash', command_02_script,
        '--input', meme_file,
        '--db', db
    ]
    result_02 = sp.run(command_02, capture_output=True, text=True)
    if result_02.returncode != 0:
        print(f"Error in run_ama_gomo_02:\n{result_02.stderr}")
        raise RuntimeError("run_ama_gomo 02 failed")

# Command 3: Run GO Annotations R script
def run_GO_Annotations_r(cluster, output, xml_file, txt_file):
    command_03_script = os.path.join(os.path.dirname(__file__), 'KmerGOn_03.R')
    command_03 = [
